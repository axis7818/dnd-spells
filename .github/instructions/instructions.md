...
Convert a JSON list of D&D spell definitions into the following markdown format:
...
Output spell definitions to individual markdown files in a new folder called `spell-output`.

The description content should contain just human-readable plaintext. The source json has marked up links. That markup should be stripped out. For example, `{@variantrule Fly Speed|XPHB}` should become just `Fly Speed`. `{@condition Invisible|XPHB}` should become `Invisible`.

## Spells in D&D

The following is a description of spellcasting in D&D:
